...
GroceryItemDatabase::GroceryItemDatabase( const std::string & filename )
{
  ...
  fin.open(filename);
  while (fin >> fileItem) dataStore.push_back(fileItem);
...
///////////////////////// TO-DO (3) //////////////////////////////
  ...
  {
    GroceryItem * begin = & (this->dataStore[0]);

    return find( begin ,isbn);
  }

  GroceryItem * GroceryItemDatabase::find(GroceryItem * current, const std::string & isbn)
    {
