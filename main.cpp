...
namespace
{
  ...
  *********************************************************************************************************************************/
  void carefully_move_grocery_items( std::size_t quantity, std::stack<GroceryItem> & broken_cart, std::stack<GroceryItem> & working_cart, std::stack<GroceryItem> & spare_cart )
  {
    ...
    if (quantity == 1)
    {
      working_cart.emplace(broken_cart.top());
      broken_cart.pop();
...
int main( int argc, char * argv[] )
{
  ...
  ///////////////////////// TO-DO (4) //////////////////////////////
    ...
    myCart.push(GroceryItem ("milk",          "Fair Life",       "00075457129000", 9.64));
    myCart.push(GroceryItem ("rice krispies", "Kellogg's",       "00038000291210", 17.58));
    myCart.push(GroceryItem ("hotdogs",       "Applegate Farms", "00025317533003", 12.57));
    myCart.push(GroceryItem ("apple pie",     "Marie",           "09073649000493"));
    myCart.push(GroceryItem ("bread",         "Farmers",         "00835841005255", 3.87));
    myCart.push(GroceryItem ("eggs",          "Chick",           "00688267039317", 24.66));
  ...
  while(workingCart.empty() != true)
  {
    checkoutCounter.emplace(workingCart.top());
    workingCart.pop();
  }
  /////////////////////// END-TO-DO (6) ////////////////////////////
  ...
  ///////////////////////// TO-DO (7) //////////////////////////////
    ...
    /// no charge.
  std::string itemUpc = checkoutCounter.front().upcCode();
  std::cout << itemUpc << "\n";
  while( checkoutCounter.empty() != true)
  {
    //Start working here. Need this to return nullptr for items not in database.
    //Need to compare upc in cart to upc in database.
    if(worldWideDatabase.find(itemUpc) == 0)
    { 
      std::cout << checkoutCounter.front() << "\n";
      amountDue += checkoutCounter.front().price();
      checkoutCounter.pop();
    } 
    else if (worldWideDatabase.find(itemUpc) == nullptr)
    {
      std::cout << "(" << checkoutCounter.front().productName() << ")" << " product not found, you get it free now!\n";
      checkoutCounter.pop();
    }
  }
  /////////////////////// END-TO-DO (7) ////////////////////////////
