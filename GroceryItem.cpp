...
std::istream & operator>>( std::istream & stream, GroceryItem & groceryItem )
{
  ///////////////////////// TO-DO (21) //////////////////////////////
    ...
    GroceryItem workItem;
    char delimiter = '\0';

    if(stream >> std::ws >> std::quoted(workItem._upcCode)     && stream >> std::ws >> delimiter && delimiter == ',' &&
       stream >> std::ws >> std::quoted(workItem._brandName)   && stream >> std::ws >> delimiter && delimiter == ',' &&
       stream >> std::ws >> std::quoted(workItem._productName) && stream >> std::ws >> delimiter && delimiter == ',' &&
       stream >> std::ws >> workItem._price)
    {
      groceryItem = std::move(workItem);
    }
